...
//   bank_name: string | null;
//   note: string | null;
//   // FIX (note #4): needed so hierarchy/permissions can be shown & edited here
...
//   // FIX (note #10): banks now come from a shared `custom_banks` DB table
//   // instead of localStorage, so every staff member/browser sees the same list.
//   const [customBanks, setCustomBanks] = useState<string[]>([]);
...
//       // quietly here rather than blocking the whole page.
//       return;
//     }

//     setCustomBanks((data ?? []).map((r) => r.name).filter(Boolean));
//   };
...
//       .on(
//         "postgres_changes",
//         { event: "*", schema: "public", table: "custom_banks" },
//         () => loadCustomBanks(),
//       )
//       .subscribe();
...
//     // FIX (note #10): persist to Supabase so this is visible to every user,
//     // not just saved in the current browser's localStorage.
//     const { error } = await supabase
//       .from("custom_banks")
...
//           ? !row.assigned_to
//           : row.assigned_to === assigneeFilter);

//       return matchesSearch && matchesStage && matchesBank && matchesAssignee;
//     });
...
//     return data;
//   }, [q, rows, stageFilter, bankFilter, assigneeFilter, sortBy]);
...
//   // FIX (note #4): admins/managers can (re)assign a customer to staff —
//   // previously there was no way to do this once a lead became a customer,
//   // so staff-scoped RLS would leave customers permanently invisible to them.
//   const updateAssignee = async (row: Row, value: string) => {
...
//     setRows((prev) =>
//       prev.map((item) =>
//         item.id === row.id ? { ...item, assigned_to: newId } : item,
//       ),
//     );
...
//           loan_amount: row.loan_amount,
//           requested_amount: row.loan_amount,
//           // FIX: use the same stage vocabulary documented for loan_cases
//           // ("Under Process" / "Sanction" / "Disbursed" / "Closed") instead
//           // of the one-off "Completed" value used nowhere else.
//           stage: "Closed",
//         });
...
//         else toast.success("Closed → Loan case created");
//       } else {
//         // A loan_case already exists (created at approval time in Leads) —
//         // keep it in sync instead of silently leaving it stale.
//         await supabase
//           .from("loan_cases")
//           .update({ stage: "Closed" })
//           .eq("customer_id", row.id);
...
//     return r.full_name?.toLowerCase().includes(s) || r.phone?.includes(q) || r.email?.toLowerCase().includes(s);
//   });
...
//           <UserCheck className="h-4 w-4" /> Customer Directory
//         </div>
//         <h1 className="text-2xl font-bold tracking-tight md:text-3xl">Converted Customers</h1>
//         <p className="text-sm text-slate-500">Auto-updated list of all converted lead customers across all verticals.</p>
//       </div>

//       <Card className="p-4">
//         <div className="relative min-w-[220px] flex-1">
//           <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
//           <Input placeholder="Search customer name, phone, email..." value={q} onChange={(e) => setQ(e.target.value)} className="pl-9" />
//         </div>
//       </Card>
...
//           </div>
//         ) : filtered.length === 0 ? (
//           <div className="p-10 text-center text-sm text-slate-500">No customers converted yet.</div>
//         ) : (
//           <Table>
//             <TableHeader>
//               <TableRow>
...
//                       {r.phone && <span className="flex items-center gap-1"><Phone className="h-3 w-3 text-slate-400" />{r.phone}</span>}
//                       {r.email && <span className="flex items-center gap-1 text-slate-500"><Mail className="h-3 w-3 text-slate-400" />{r.email}</span>}
//                     </div>
//                   </TableCell>
//                   <TableCell>{r.city ?? "—"}</TableCell>
//                   <TableCell className="capitalize text-xs font-semibold">{r.product_type?.replace("_", " ")}</TableCell>
//                   <TableCell>
...
>>>>>>> ef512b67628c9f23bd4dce4bc5838e826a816535
  ...
  // Customer ka naam nikalne ka robust helper function
  const getCustomerName = (r: Customer) => {
    if (r.full_name && r.full_name.trim() !== "") return r.full_name;
    if (r.name && r.name.trim() !== "") return r.name;
    if (r.customer_name && r.customer_name.trim() !== "") return r.customer_name;
    ...
    <div className="space-y-4 p-4 md:p-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-500">
          Auto-updated list of all converted lead customers across all verticals.
        </p>
      </div>
                  ...
                  return (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="capitalize text-xs font-semibold">
                        {r.product_type ? r.product_type.replace("_", " ") : "—"}
                      </TableCell>
                      <TableCell>
                        <Badge className="bg-emerald-100 text-emerald-800 border-0">
                          {r.status || "Active Customer"}
                        </Badge>
                      </TableCell>
