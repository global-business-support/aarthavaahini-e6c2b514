...
function DirectorCard({
  ...
  const imageOrderClass =
    director.imagePosition === "right" ? "order-1 md:order-2" : "order-1";

  const contentOrderClass =
    director.imagePosition === "right" ? "order-2 md:order-1" : "order-2";

  const imageBlock = (
    <div className={`flex items-center justify-center p-4 ${imageOrderClass}`}>
      <div className="h-[300px] w-full max-w-[420px] overflow-hidden rounded-xl border border-slate-200 bg-[#f8fbff] p-2 shadow-sm">
        <img
          src={director.image}
          alt={director.name}
  ...
  const contentBlock = (
    <div className={`flex flex-col justify-center p-6 sm:p-8 ${contentOrderClass}`}>
      <h2 className="font-display text-3xl font-bold leading-tight text-[#07142f] sm:text-4xl">
        {director.name}
      </h2>
