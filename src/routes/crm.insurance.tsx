...
function InsurancePage() {
  ...
  const fetchInsuranceCases = async () => {
    setLoading(true);
  ...
  const submitConsultation = async (e: React.FormEvent) => {
    ...
    if (error) {
      setSaving(false);
      console.error("Insurance consultation lead error:", error);
    ...
    if (lead?.id) {
      await supabase.from("activities").insert({
        ...
        notes:
          form.notes?.trim() ||
          `Free consultation request created from Insurance page${
            form.policy_type ? ` for ${form.policy_type}` : ""
          }.`,
        lead_id: lead.id,
        customer_id: null,
      });
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="border-sky-100 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-slate-500">Total Insurance Cases</p>
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center">
            <p className="text-sm text-slate-500">No insurance cases yet.</p>
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    {r.premium
                      ? `₹${Number(r.premium).toLocaleString("en-IN")}`
                      : "—"}
                  </TableCell>
                  <TableCell>
                    <Badge variant="secondary">{r.policy_status}</Badge>
                  </TableCell>
                  <TableCell>{r.renewal_date ?? "—"}</TableCell>
                </TableRow>
              ))}
            ...
            <div className="flex items-center justify-between border-b px-5 py-4">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Create an insurance consultation lead
                </p>
              </div>
            ...
            <form onSubmit={submitConsultation} className="space-y-5 p-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  value={form.notes}
                  onChange={handleChange}
                  placeholder="Customer requirement, policy interest, premium budget etc."
