...
function DashboardPage() {
  ...
  const [overrides, setOverrides] = useState<
    Record<string, { label?: string; value?: string | null; trend?: string | null }>
  >({});
  ...
  const loadDashboard = useCallback(async () => {
    ...
    const [
      ...
      supabase
        .from("loan_cases")
        .select("loan_amount, requested_amount, stage")
        .not("stage", "in", '("Closed","Dropped","Rejected")'),

      supabase
        .from("insurance_cases")
        .select("premium, policy_status")
        .not("policy_status", "in", '("Issued","Closed","Dropped","Rejected")'),

      supabase
        .from("mutual_funds")
        .select("sip_amount, status")
        .not("status", "in", '("Portfolio Review","Closed","Dropped","Rejected")'),
      ...
      supabase
        .from("loan_cases")
        .select("stage, requested_amount, sanction_amount, disbursement_amount")
        .not("stage", "in", '("Closed","Dropped","Rejected")'),
    ]);

    const sum = <T extends Record<string, unknown>>(
      rows: T[] | null,
      key: string,
    ) => {
    ...
    const loanPipelineAmount = ((loans.data ?? []) as LoanPipelineRow[]).reduce(
      (acc, row) => {
        return (
          acc +
          (Number(row.loan_amount) || Number(row.requested_amount) || 0)
        );
      },
      0,
    );
    ...
    (last30.data ?? []).forEach((row: { created_at: string }) => {
    ...
    (custStage.data ?? []).forEach((customer: { stage: string | null }) => {
      const stage = customer.stage || "New";
      stageMap[stage] = (stageMap[stage] || 0) + 1;
    });

    setCustomerStages(
      Object.entries(stageMap).map(([name, value]) => ({
        name,
        value,
      })),
    );
    ...
    const loanMap: Record<
      string,
      { requested: number; sanctioned: number; disbursed: number }
    > = {};

    ((loanRows.data ?? []) as LoanStageRow[]).forEach((row) => {
    ...
    setLoanByStage(
      Object.entries(loanMap).map(([stage, value]) => ({
        stage,
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select("key,label,value_override,trend")
      .then(({ data }) => {
        if (!data) return;
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "mutual_funds" },
        () => loadDashboard(),
      )
      .subscribe();
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-white/85">
              Realtime overview — leads, customers, loans synced live
            </div>
          </div>
        </div>
          ...
          return (
            ...
            >
              <Card className="relative overflow-hidden border-sky-100 bg-white/85 p-4 shadow-sm backdrop-blur-sm transition-all hover:-translate-y-0.5 hover:shadow-md hover:ring-2 hover:ring-sky-200">
                ...
                <div className="relative flex items-start justify-between">
                  <div className={cn("rounded-xl p-2 shadow-sm", toneBg(card.tone))}>
                    <Icon className={cn("h-4 w-4", toneFg(card.tone))} />
                  </div>

                  <Badge
                    variant="secondary"
                    className="bg-sky-50 text-[10px] font-medium text-sky-700"
                  >
                    {card.trend}
                  </Badge>
                </div>
                ...
                <ArrowUpRight className="absolute bottom-3 right-3 h-3.5 w-3.5 text-slate-300 transition group-hover:text-sky-500" />
              </Card>
            </Link>
          );
        })}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {/* Leads */}
        <Link to="/crm/leads" className="block">
          <Card className="p-5 transition hover:-translate-y-0.5 hover:shadow-md hover:ring-2 hover:ring-sky-200 lg:col-span-1">
            <div className="flex items-center justify-between">
              <div>
        ...
        {/* Loans */}
        <Link to="/crm/loans" className="block">
          <Card className="p-5 transition hover:-translate-y-0.5 hover:shadow-md hover:ring-2 hover:ring-emerald-200 lg:col-span-1">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-sm font-semibold text-slate-900">
                  Loans · ₹ Lakh by Stage
                ...
                <p className="text-xs text-slate-500">
                  Requested · Sanctioned · Disbursed.
                </p>
              </div>
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Click a name to open the unified profile.
            </p>
          </div>
        ...
        <div className="mt-3 divide-y divide-sky-50">
          ...
          {recentLeads.map((lead) => (
            ...
            >
              <button
                type="button"
                onClick={() => setProfileLead(lead.id)}
              ...
              >
                ...
                <div>
                  <div className="text-sm font-medium text-sky-700 group-hover:underline">
                    {lead.full_name}
                  </div>

                  <div className="text-xs capitalize text-slate-500">
                    {lead.product_type?.replace(/_/g, " ")}
                  </div>
                </div>
              </button>
      ...
      <CustomerProfileDialog
        open={!!profileLead}
        onOpenChange={(value) => !value && setProfileLead(null)}
        leadId={profileLead}
      />
    </div>
  );
