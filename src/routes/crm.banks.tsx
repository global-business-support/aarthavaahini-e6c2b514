...
function BanksPage() {
  ...
  const importWebsiteBanks = async () => {
    if (
      !confirm(
        `Import ${WEBSITE_BANKS.length} banks from the website list? Existing banks with the same name will be skipped.`,
      )
    )
      return;
    ...
    const toInsert = WEBSITE_BANKS.filter((b) => !existing.has(b.name.toLowerCase())).map(
      (b, i) => ({
        name: b.name,
        domain: b.domain,
        logo_url: b.logo ?? `https://logo.clearbit.com/${b.domain}`,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            onClick={importWebsiteBanks}
            variant="outline"
            className="rounded-xl border-slate-300 bg-white shadow-sm hover:bg-sky-50 hover:text-sky-700"
          >
          ...
          <Button
            onClick={openNew}
            className="rounded-xl bg-sky-600 shadow-sm hover:bg-sky-700"
          >
      ...
      <Card className="rounded-2xl border-slate-200 bg-white/80 p-4 shadow-sm backdrop-blur">
        <div className="relative mb-4 max-w-sm">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder="Search banks…"
            className="h-11 rounded-xl border-slate-300 bg-white pl-9 shadow-sm focus-visible:ring-sky-500"
          />
        </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filtered.map((b) => (
              <div
                key={b.id}
                className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-lg"
              >
                <div className="flex items-start gap-3">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-sky-50 ring-1 ring-sky-100">
                    {b.logo_url ? (
                      <img
                        src={b.logo_url}
                        alt={b.name}
                ...
                <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                  ...
                  <div className="flex gap-1">
                    <Button
                      ...
                      variant="ghost"
                      onClick={() => openEdit(b)}
                      className="h-8 w-8 rounded-lg hover:bg-sky-50 hover:text-sky-700"
                    >
                      <Pencil className="h-4 w-4" />
                    </Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="overflow-hidden rounded-3xl border-0 bg-white p-0 shadow-2xl sm:max-w-[620px]">
          <DialogHeader className="relative border-b border-sky-100 bg-gradient-to-r from-sky-50 via-blue-50 to-cyan-50 px-6 py-5">
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="absolute right-4 top-4 inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/80 text-slate-500 shadow-sm ring-1 ring-slate-200 transition hover:bg-white hover:text-slate-900"
            >
              <X className="h-4 w-4" />
            </button>
            ...
            <div className="flex items-center gap-3 pr-10">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  {editing
                    ? "Update bank details shown in CRM and public forms."
                    : "Create a new bank or NBFC partner for CRM forms."}
                </p>
              </div>
            </div>
          </DialogHeader>
            ...
            <div className="grid gap-5">
              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                ...
                <div className="grid gap-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      <Label className="mb-1.5 flex items-center gap-1.5 text-sm font-medium text-slate-700">
                        <Tags className="h-3.5 w-3.5 text-sky-600" />
                        Category
                      </Label>
                      <Input
                        value={form.category ?? ""}
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="flex flex-col gap-4 sm:flex-row">
                  <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-sky-100 bg-sky-50">
                    {form.logo_url ? (
                      <img
                        src={form.logo_url}
                        alt="preview"
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-1.5 text-xs text-slate-500">
                      Paste a logo image URL or keep blank if you want to use domain-based logo.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-sky-50 p-4 shadow-sm">
                  ...
                  <div className="flex items-center justify-between rounded-xl border border-sky-100 bg-white px-4 py-3">
                    <div>
                      <p className="text-sm font-semibold text-slate-900">Active</p>
                      <p className="text-xs text-slate-500">
                        Show this bank in CRM forms
                      </p>
                    </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <Textarea
                  ...
                  onChange={(e) => setForm({ ...form, notes: e.target.value })}
                  placeholder="Internal notes (commission, contact person, etc.)"
                  className="resize-none rounded-xl border-slate-300 bg-white shadow-sm focus-visible:ring-sky-500"
                />
              </div>
            </div>
          </div>
          ...
          <DialogFooter className="border-t border-slate-100 bg-slate-50/80 px-6 py-4">
            <Button
              variant="outline"
              onClick={() => setOpen(false)}
              className="h-11 rounded-xl border-slate-300 bg-white px-5 shadow-sm hover:bg-slate-100"
            >
              Cancel
            </Button>
            ...
            <Button
              onClick={save}
              className="h-11 rounded-xl bg-sky-600 px-6 font-semibold shadow-md shadow-sky-200 hover:bg-sky-700"
            >
              {editing ? "Save Changes" : "Add Bank"}
            </Button>
          </DialogFooter>
